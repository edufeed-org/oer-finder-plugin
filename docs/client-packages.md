...
1. **[API Client](#api-client-package)** (`@edufeed-org/oer-finder-api-client`) - Type-safe TypeScript client for direct API access
2. **[Web Components Plugin](#web-components-plugin)** (`@edufeed-org/oer-finder-plugin`) - Ready-to-use web components built with Lit
3. **[React Components](#react-components-package)** (`@edufeed-org/oer-finder-plugin-react`) - React wrapper components for the web components plugin
...
1. **[Server-Proxy mode](#server-proxy-mode)** — When `api-url` is provided, all requests go through the proxy backend. The server handles all adapter logic. **No adapter registration or imports are needed on the client side** — no adapter code is bundled into your application, keeping the client bundle small.
2. **[Direct Client mode](#direct-client-mode)** — When `api-url` is *not* provided, adapters run directly in the browser. No server needed. **You must register adapters before the first search.**

In both modes, use the `sources` JS property to configure which sources are available in the UI (labels, checkboxes, pre-selection). See [Source Configuration](#source-configuration) for details.

### Server-Proxy Mode

Set the `api-url` attribute to your proxy server. No adapter registration or imports needed on the client side.
...
<script type="module">
  ...
  // Configure which sources appear in the UI
...
Omit `api-url` — adapters run directly in the browser. You must **register adapters before the first search**.
...
// Or selectively (reduces bundle size)
...
**2. Use the components without `api-url`:**
...
<script type="module">
  ...
  // Configure sources — provide baseUrl where required (e.g., nostr-amb-relay)
  searchElement.sources = [
  ...
  // Event handling is identical to server-proxy mode (see above)
</script>
```
