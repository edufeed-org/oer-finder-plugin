...
- Multi-source search with round-robin interleaving and per-source page tracking
- i18n support (English and German)
- Themeable via CSS custom properties

## Key Files

- `src/oer-search/OerSearch.ts` — Main search component, manages client creation and multi-source search
...
- Interleaving logic is pure functional in `src/interleave.ts`; per-source page state is tracked in `OerSearch.ts`
